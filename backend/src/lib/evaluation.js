...
function evaluateFairThreat(ale, cvar95, criteria, formatCurrency) {
    ...
    if (ale > aleCritico) {
        return {
            level: 'Crítico — Requiere Acción Inmediata', severity: 'critico',
    ...
    if (cvar95 > aleCritico) {
        return {
            level: 'Crítico (riesgo de cola) — Requiere Atención', severity: 'critico',
    ...
    if (ale > aleAceptable) {
        return {
            level: 'Requiere Tratamiento', severity: 'alto',
...
function evaluateFairOpportunity(benefit, criteria, formatCurrency) {
    ...
    if (benefit > aleAceptable) {
        return {
            level: 'Oportunidad Moderada — Considerar Perseguir', severity: 'medio',
    ...
    return {
        level: 'Oportunidad Menor', severity: 'bajo',
